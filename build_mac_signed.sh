...
message=":hammer_and_wrench: ARM MacOS build started on \`$BRANCH_NAME\` with $analyticsMessage analytics. It will be ready approximately at $end_time"
...
execute_file_upload "${SLACK_BOT_TOKEN}" "${SLACK_CHANNEL}" ":white_check_mark: ARM MacOS from \`$BRANCH_NAME\` with ${analyticsMessage} analytics" "upload" "${NEW_PKG_PATH}"
