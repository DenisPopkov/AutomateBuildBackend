...
message=":hammer_and_wrench: ARM MacOS build started on \`$BRANCH_NAME\`
...
execute_file_upload "${SLACK_BOT_TOKEN}" "${SLACK_CHANNEL}" ":white_check_mark: ARM MacOS signed from \`$BRANCH_NAME\` (analytics=${analyticsMessage})" "upload" "${NEW_PKG_PATH}"
