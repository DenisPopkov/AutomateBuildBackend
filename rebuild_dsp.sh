...
message=":hammer_and_wrench: Start ARM MacOS DSP update on \`$BRANCH_NAME\` for $ENV"
...
rm -rf "$HEROKU_LIBRARY_PATH/arm64/libdspmac.dylib"

if [ -f "$CACHE_UPDATED_DSP_LIB_PATH" ]; then
  cp "$CACHE_UPDATED_DSP_LIB_PATH" "$HEROKU_LIBRARY_PATH/arm64/"
else
...
message=":white_check_mark: ARM MacOS DSP library successfully updated on \`$BRANCH_NAME\` for $ENV"
